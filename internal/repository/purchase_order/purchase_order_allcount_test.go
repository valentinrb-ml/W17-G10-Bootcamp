...
func TestPurchaseOrderRepository_GetAllWithPurchaseCount(t *testing.T) {
	tests := []struct {
		...
		{
			name: "success - multiple buyers",
			setup: func(mock sqlmock.Sqlmock) {
				rows := sqlmock.NewRows([]string{"id", "id_card_number", "first_name", "last_name", "purchase_orders_count"}).
					AddRow(101, "CARD101", "John", "Doe", 3).
					AddRow(102, "CARD102", "Jane", "Smith", 5)
				mock.ExpectQuery("SELECT b.id, b.id_card_number, b.first_name, b.last_name, COUNT\\(po.id\\) as purchase_orders_count FROM buyers b LEFT JOIN purchase_orders po ON b.id = po.buyer_id GROUP BY b.id").
					WillReturnRows(rows)
			},
			want: []models.BuyerWithPurchaseCount{
		...
		},
		{
			...
			wantErr:        true,
			expectedErrMsg: "error querying all purchase counts",
		},
	}
