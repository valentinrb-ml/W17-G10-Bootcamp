...
func (r *productBatchesRepository) CreateProductBatches(ctx context.Context, proBa models.ProductBatches) (*models.ProductBatches, error) {
	if r.logger != nil {
		r.logger.Info(ctx, "product-batches-repository", "Creating product batch", map[string]interface{}{
			"product_batch": proBa,
		})
	}
	...
	if err != nil {
		...
		if errors.As(err, &mysqlErr) && mysqlErr.Number == 1062 {
			if r.logger != nil {
				r.logger.Error(ctx, "product-batches-repository", "Failed to create product batch", err)
			}
	...
	if err != nil {
		if r.logger != nil {
			r.logger.Error(ctx, "product-batches-repository", "Failed to get last insert id", err)
		}
		return nil, err
	}
...
func (r *productBatchesRepository) GetReportProductById(ctx context.Context, id int) (*models.ReportProduct, error) {
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			if r.logger != nil {
				r.logger.Error(ctx, "product-batches-repository", "Section not found", err)
			}
			return nil, apperrors.NewAppError(apperrors.CodeNotFound, "The section you are looking for does not exist.")
		}
		if r.logger != nil {
			r.logger.Error(ctx, "product-batches-repository", "Failed to get report product by id", err)
		}
...
func (r *productBatchesRepository) GetReportProduct(ctx context.Context) ([]models.ReportProduct, error) {
	if r.logger != nil {
		r.logger.Info(ctx, "product-batches-repository", "Getting report product")
	}
	rows, err := r.mysql.QueryContext(ctx, queryGetProductsReport)
	if err != nil {
		if r.logger != nil {
			r.logger.Error(ctx, "product-batches-repository", "Failed to get report product", err)
		}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&rp.SectionId, &rp.SectionNumber, &rp.ProductsCount); err != nil {
			if r.logger != nil {
				r.logger.Error(ctx, "product-batches-repository", "Failed to scan report product", err)
			}
			return nil, apperrors.NewAppError(apperrors.CodeInternal, "An internal server error occurred while retrieving the product report.")
		}
