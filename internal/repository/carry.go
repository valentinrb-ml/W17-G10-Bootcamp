...
const (
	...
	queryCarriesCountByAllLocalities = `SELECT c.locality_id, l.locality_name, COUNT(*) as carries_count FROM carriers c INNER JOIN localities l ON c.locality_id = l.id GROUP BY c.locality_id`
	queryCarriesCountByLocalityID = `SELECT c.locality_id, l.locality_name, COUNT(*) as carries_count FROM carriers c INNER JOIN localities l ON c.locality_id = l.id WHERE c.locality_id = ? GROUP BY c.locality_id`
)
...
func (r *CarryMySQL) GetCarriesCountByLocalityID(ctx context.Context, localityID int) (*carry.CarriesReport, error) {
	var cc carry.CarriesReport
	...
	if err != nil {
		if mysqlErr, ok := err.(*mysql.MySQLError); ok {
			switch mysqlErr.Number {
			case 1452:
				return nil, apperrors.NewAppError(apperrors.CodeNotFound, "locality_id does not exist")
			}
		}
